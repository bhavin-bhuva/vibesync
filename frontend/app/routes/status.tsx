...
import { StatusViewer, type StatusContent } from "../components/status/status-viewer";

export function meta() {
  return [
    { title: "Status - VibeSync" },
    { name: "description", content: "View and share your status on VibeSync" },
  ];
}
...
export default function Status() {
  ...
  return (
    <div className="h-screen flex flex-col glass-dark">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto">
        {/* My Status */}
        <div className="p-4 border-b border-white/10">
          <h3 className="text-sm font-semibold text-gray-400 mb-3">My Status</h3>
          <button className="w-full flex items-center gap-3 hover:bg-white/5 p-3 rounded-lg transition-colors">
            ...
            <div className="flex-1 text-left">
              <h4 className="font-medium text-white">My Status</h4>
            ...
            <div className="space-y-2">
              {recentStatuses.map((status) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 hover:bg-white/5 p-3 rounded-lg transition-colors"
                >
                  <div className="relative">
                    <div
            ...
            <div className="space-y-2">
              {viewedStatuses.map((status) => (
                <button
                  key={status.id}
                  onClick={() => handleStatusClick(status)}
                  className="w-full flex items-center gap-3 hover:bg-white/5 p-3 rounded-lg transition-colors opacity-75"
                >
                  <div className="relative">
                    <div
                      className="w-14 h-14 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-white font-semibold ring-2 ring-gray-600"
                    >
                      {status.userName
                        .split(" ")
                        .map((n) => n[0])
